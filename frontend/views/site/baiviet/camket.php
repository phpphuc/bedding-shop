...
if ($chamsoc) {
    ...
    if ($modelCategory) {
        ...
        $newAll = Posts::find()->where('status = 1 and parent IN (' . implode(',', $modelCategory) . ')')->limit(4)->orderBy('number asc, id desc')->asArray()->all();
    } else {

        $newAll = Posts::find()->where('status = 1 and parent = ' . $chamsoc['id'])->limit(4)->orderBy('number asc, id desc')->asArray()->all();
    }
    ?>
    <div class="khungcstrong">
        <div class="pagewrap" data-aos="fade-right" data-aos-duration="2000">
            <div class="row">
                <?php
                if ($newAll) {
                    foreach ($newAll as $items) :
                        ...
                        $imgN = $items['image'];
                        ?>
                        <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 padding-left-10 padding-right-10">
                            <div class="row">
                                <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 padding-left-0 padding-right-5">
